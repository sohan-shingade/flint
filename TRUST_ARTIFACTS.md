...
| 1.2 | Parity report pipeline + CI gate | 🟡 Partial — `scripts/run_parity_report.py` emits markdown artifacts with threshold gate; 6 strategies catalogued. CI gate wiring = Phase 5.3. | [phase-1 §1.2](docs/specs/phase-1-trust-correctness.md#12-parity-report-pipeline) | — | TBD | 5.3 |
...
| 1.4 | Reconciliation tooling (live fills vs engine fills, UI mismatch report) | 🟡 Partial — `scripts/reconcile_fills.py` matches engine vs venue fills; markdown report with price-bps / ts p50/p95/p99. API endpoint + UI panel = Phase 4. 14 tests. | [phase-1 §1.4](docs/specs/phase-1-trust-correctness.md#14-reconciliation-tooling) | — | TBD | Phase 4 |
| 1.5 | Proof notebooks per flagship strategy | 🟢 Shipped — `notebooks/funding_arb.py`, `basis_trade.py`, `momentum_breakout.py` (jupytext-compatible), each pins candle sha256, runs backtest + parity, emits artifact, CI-gated via exit code. `notebooks/README.md`. | [phase-1 §1.5](docs/specs/phase-1-trust-correctness.md#15-proof-notebooks) | — | — | — |
