<p align="center">
  ...
  <br/><br/>
  <strong>The best local backtester and paper-trading lab for Drift and Hyperliquid perp strategies. Realistic fills, funding-aware margin, cross-venue arb — one command to install, nothing leaves your machine.</strong>
  <br/><br/>
...
<!-- counts:auto -->
- **23 built-in strategies** · **26 data providers** · **81 REST endpoints** · **20 MCP tools** · **2102 tests**
...
Flint is a **local-first quant lab focused on Drift + Hyperliquid perp strategies**. Write a strategy in Python, backtest it against real on-chain candles and funding rates with venue-accurate fills + margin + liquidation, then paper trade with live WebSocket data. When you're ready, the same strategy code runs against Drift devnet/mainnet and Hyperliquid testnet/mainnet via one CLI command.

Market data for the wedge (Drift + Hyperliquid + Pyth) is free — those venues publish OHLCV, funding, open interest, and orderbook data via public APIs. Optional providers (Birdeye, Helius, CCXT, Tardis) need their own keys. Everything is cached in a local DuckDB file. Nothing leaves your machine unless a data provider needs it to — no telemetry, no cloud sync.

<p align="center">
  <img src="imgs/homepage.png" alt="Flint homepage" width="100%">
</p>

### Why Flint (vs DeFi-native tools)

| | Flint | Anything else for DeFi perps |
|---|:---:|:---:|
| **Drift vAMM fill modeling** (peg multiplier, oracle-anchored, per-market `sqrt_k`) | ✓ | — |
| **Hyperliquid CLOB walk + HLP backstop** | ✓ | — |
| **Cross-venue funding dislocation arb (Drift + HL)** with shared-capital backtest | ✓ | — |
| **Per-venue margin + liquidation simulation** | ✓ | — |
| **Slippage calibration from live fills** | ✓ | — |
| **Monte Carlo bootstrap (trade-frequency Sharpe)** | ✓ | — |

### Why Flint (vs general crypto bots)

| | Flint | Freqtrade | Hummingbot |
|---|:---:|:---:|:---:|
| **Built for DeFi perps first** | ✓ | — | Partial |
| **Free data for the wedge** | ✓ (Drift/HL/Pyth) | — | — |
| **Paper trading with live data** | ✓ | ✓ | ✓ |
| **Browser UI + Monaco editor** | ✓ | — | — |
| **Optuna optimization** | ✓ | Hyperopt | — |
| **MCP server (AI integration)** | ✓ | — | — |
| **Local-first (no cloud)** | ✓ | ✓ | ✓ |
| **CCXT data for cross-venue analysis** | Read-only today | ✓ | ✓ |
| **CEX live order submission** | Planned | ✓ | ✓ |

Freqtrade is excellent for CEX spot bots. Flint is what you use when your alpha lives on Drift and Hyperliquid perps and you need an engine that models how those venues actually work.
...
Three proof notebooks (jupytext-format Python files, runnable as scripts or openable in Jupyter):

```bash
python notebooks/funding_arb.py           # cross-venue funding capture, 30-day window
python notebooks/basis_trade.py           # perp-vs-spot basis, 30 days
python notebooks/momentum_breakout.py     # directional baseline, 90 days
```

Each notebook pins the candle sha256, runs backtest + paper replay on the same data, and emits a parity report at `artifacts/parity/{strategy}-{market}-{date}.md`. CI fails if PnL divergence exceeds 2% of initial capital — that's the acceptance gate Drift/HL firms ask for, shipped as a script anyone can re-run.

For ad-hoc workflows:

```bash
python scripts/run_parity_report.py funding_momentum_v4 SOL-PERP 30   # parity report
python scripts/reconcile_fills.py --session <id> --fills-csv my.csv   # live vs engine
python scripts/audit_pit.py                                           # PIT audit
python scripts/calibrate.py --fills my.csv --market SOL-PERP ...      # slippage fit
```

See [Known Limitations](docs/validation/known-limitations.md) for what the backtester does and does not model, and [TRUST_ARTIFACTS.md](TRUST_ARTIFACTS.md) for the live status of Phase 1 reproducibility work.
...
- **Live PnL updates** -- DLOB mid-price polling every 5 seconds + per-session WebSocket stream (`/ws/paper/{id}`)
...
- **Time-travel replay** -- every order, fill, funding payment, and liquidation written to a per-session event log; rebuild the book at any past timestamp via the Replay page or `/api/v1/replay/{id}/state`
...
20 tools across backtesting, paper trading, data, optimization, journal, and replay. Key tools: `run_backtest`, `start_paper_trading`, `get_paper_sessions`, `optimize_strategy`, `download_market_data`, `list_journal_runs`, `get_candles`, `get_funding_rates`, plus time-travel replay tools `replay_summary`, `replay_state`, `list_replay_events` for reconstructing any session's book at any past timestamp.
