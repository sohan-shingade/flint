...
- **Price history** — real OHLCV from Drift / Hyperliquid / 100+ CEX via CCXT. No synthesized data.
- **Per-venue fees** — taker/maker rates from actual venue schedules. Maker rebates on Drift (−2 bps) feed back into PnL.
- **Per-venue funding** — hourly rates from 7 perp venues, forward-filled from the real 8h schedule.
- **Slippage** — 4-tier fill pipeline (vAMM / orderbook walk / sqrt impact / flat) chosen per order per bar.
- **Latency** — per-venue `base_latency ± jitter`. Drift's 8s delay meaningfully affects short-timeframe strategies.
...
- **Venue downtime.** Drift outages, Hyperliquid halts, CEX maintenance windows. Backtests fill as if the venue is always up.
...
- **Strategy frequency.** Intraday momentum on Drift — large gap (MEV + latency + slippage). Weekly rebalance — small gap.
...
| Devnet testing | Venue-specific surprises | Drift devnet / Hyperliquid testnet before mainnet |
