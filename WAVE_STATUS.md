...
| ID | State | Effort | Prereq | Notes |
|---|---|---|---|---|
| D-6.4-replay | 🟡 | 2-3w | D-2.2-internal ✓ | Slices 1+2+3 shipped: event log writer/reader + `BookState` fold/replay + snapshot compaction with fast-forward replay path. 47 tests across `event_log`/`replay`/`snapshots`. Engine writer hooks (slice 4) + time-travel UI (slice 5) deferred |
