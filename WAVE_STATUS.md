...
|---|---|---|---|---|---|
| D-2.1.b | 🟡 | 2w | claude | 2026-04-24 | All 7 state-extraction steps shipped (`PositionManager`, `CashManager`, `FillRecorder`, `OrderQueue`, `FundingLedger`, `BorrowLedger`, `MarketDataFeed`); BacktestContext now composes all 7 via property aliases · BacktestContext LOC still 1088 because the big method bodies (`_apply_fill`, `apply_funding`, `check_liquidations`) keep their inline call sites — caller-site migration is a follow-on PR |
