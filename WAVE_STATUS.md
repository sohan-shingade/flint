...
| ID | State | Effort | Prereq | Notes |
|---|---|---|---|---|
| D-6.4-replay | 🟡 | 2-3w | D-2.2-internal ✓ | Foundation shipped: `flint/portfolio/event_log.py:EventLogWriter` + `EventLogReader` against new `portfolio_events(session_id, seq, ts, kind, payload)` table; 6 EventKinds (order.submit/cancel/fill/liquidation/funding/borrow); thread-safe append with monotonic per-session seq; recovers MAX(seq)+1 on process restart. 15 tests including 8-thread concurrency. Snapshot + replay primitives + UI + engine writer hooks deferred to follow-on slices |
