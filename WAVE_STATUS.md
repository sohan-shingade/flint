...
| ID | State | Effort | Prereq | Notes |
|---|---|---|---|---|
| D-6.4-replay | 🟡 | 2-3w | D-2.2-internal ✓ | Foundation + replay primitive shipped: `flint/portfolio/event_log.py` (writer + reader, 6 EventKinds, thread-safe monotonic seq) + `flint/portfolio/replay.py` (`fold(events, initial_capital) → BookState` + `replay(store, session_id, target_ts, initial_capital)`). 32 tests across both modules. Snapshot compaction + UI + engine writer hooks deferred |
